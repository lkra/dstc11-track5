...
# Default task type as "pytorch"
task_type=${task_type:-"pytorch"}

# Create directories for intermediate results of each subtask
eval_dataset=val
mkdir -p pred/${eval_dataset}

# subtask 1
# turn detection
if [ "$task_type" = "pytorch" ]; then
  ...
  td_output_file=pred/${eval_dataset}/baseline.td.paddle.json
fi

# TODO make adjustments to outputs inputs so they can be read by both model types
...
# knowledge selection
  ...
  em_output_file=pred/${eval_dataset}/baseline.em.paddle.json
fi

# TODO make adjustments to outputs inputs so they can be read by both model type
